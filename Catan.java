...
//		System.out.println("this road is owned by "+roadh.getOwner().getName());
		...
		Intersection intA = (Intersection) game1.getBoard().getLocationFromCoordinate(road1.getCoordinateA()).getContains();
		Intersection intB = (Intersection) game1.getBoard().getLocationFromCoordinate(road1.getCoordinateB()).getContains();
...
//			visitorsMap.put(intA, id);
			...
			if((end1A!=null||end1B!=null)){	
				//putting the ids in a map, getting them the right way around(pointless?)
				ArrayList<Integer> idArray1 = new ArrayList<Integer>();
				...
				if (end1A!=null){
					...
					toplen = Branch(game1,player,getOtherInt(board1,end1A,intA),intA,end1A,(ArrayList<Integer>) namesList.clone(),toplen+1,distancesMap,visitorsMap);
				}
				//looking at the other branch possibility
				if (end1B!=null){
					//System.out.println("B");
					toplen = Branch(game1,player,getOtherInt(board1,end1B,intA),intA,end1B,(ArrayList<Integer>) namesList.clone(),botLen+1,distancesMap,visitorsMap);					
				}
				...
				if (end2A!=null){
					System.out.println("A");
					toplen = Branch(game1,player,getOtherInt(board1,end2A,intB),intB,end2A,(ArrayList<Integer>) namesList.clone(),toplen+1,distancesMap,visitorsMap);
				}
				//looking at the other branch possibility
				if (end2B!=null){
					System.out.println("B");
					botLen = Branch(game1,player,getOtherInt(board1,end2B,intB),intB,end2B,(ArrayList<Integer>) namesList.clone(),toplen+1,distancesMap,visitorsMap);
				}
				//getting which one of those is better!
	...
	public static int Branch(Game game1, Player player, Intersection pointInt, Intersection fromInt, Road carrying, ArrayList<Integer> idArray, int currdist, HashMap distancesMap, HashMap visitorsMap){
		System.out.println("branching to "+pointInt.getCoordinate().getX()+","+pointInt.getCoordinate().getY()+" from "+fromInt.getCoordinate().getX()+","+fromInt.getCoordinate().getY());
		int thisId = ++id;
		idArray.add(thisId);
		...
		Road option2 = getRoadFromInt(game1,pointInt,player,fromInt,getOtherInt(game1.getBoard(),option1,pointInt));
		//add branch!!!
		int option1Result=Branch( game1,  player, getOtherInt(game1.getBoard(),option1,pointInt),  pointInt, option1, (ArrayList<Integer>) idArray.clone(),  currdist+1,  distancesMap,  visitorsMap);
		if  (option2==null){
			return option1Result;
		}
		int option2Result=Branch( game1,  player, getOtherInt(game1.getBoard(),option2,pointInt),  pointInt, option2, (ArrayList<Integer>)  idArray.clone(),  currdist+1,  distancesMap,  visitorsMap);
	...
	public static Road getRoadFromInt(Game game1, Intersection sett1,Player player, Intersection int1Ill, Intersection int2Ill){
		...
		if((startRoad1!=null&&startRoad1.getOwner().equals(player))
				&&((int1Ill==null)||!int1Ill.equals((Intersection)game1.getBoard().getLocationFromCoordinate(new Coordinate(j,g+1)).getContains()))
				&&((int2Ill==null)||!int2Ill.equals((Intersection)game1.getBoard().getLocationFromCoordinate(new Coordinate(j,g+1)).getContains()))){
...
//			System.out.println(!((int2Ill==null)||int2Ill.equals((Intersection)game1.getBoard().getLocationFromCoordinate(new Coordinate(j-1,g)).getContains())));}
