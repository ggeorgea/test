...
//-----Methods to initialise players and their order for the game-----//
	...
	public static int requestPlayers(Scanner scanner, int clientLimit) {
		...
		Catan.out1.print("\n"+"How many players want to play? Enter 3 or 4. You cannot have more clients than players");
		int noPlayers = Catan.getNextInt();
		//makes sure the player inputs the correct value
		if (!(noPlayers == 3 || noPlayers == 4)) {
			Catan.out1.print("\n"+"Invalid number of players. Please choose again.");
			return requestPlayers(scanner,  clientLimit);
			
		}
		else if (noPlayers < clientLimit) {
			Catan.out1.print("\n"+noPlayers + ">" + clientLimit + ", you cannot have more clients than players ");
			return requestPlayers(scanner,  clientLimit);
		}
	...
	public static int requestClients(Scanner scanner) {
		...
		Catan.out1.print("\n"+"How many Clients should connect? 0 - 4.");

		int noPlayers = Catan.getNextInt();

		//makes sure the player inputs the correct value
		if (!(noPlayers == 3 || noPlayers == 4 || noPlayers == 1|| noPlayers == 2 || noPlayers == 0)) {
			Catan.out1.print("\n"+"Invalid number of clients. Please choose again.");
...
//-----Methods to get the initial placement of roads and settlements for the game-----//
	...
	public static void setInitialRoadsAndSettlements(Game game1, Scanner scanner) throws IOException {
		...
		ArrayList<Player> players = game1.getPlayers();
		projectX.Board board1 = game1.getBoard();
	...
	public static boolean checkNear(projectX.Board board1, int x1, int y1, int x2, int y2) {
...
public static Road placeRoad(Player player, projectX.Board board1, Scanner scanner, Game game1) throws IOException {
...
public static Intersection placeSettlement(Player player, Road road, projectX.Board board1, Scanner scanner, Game game1) throws IOException {
...
public static ArrayList<ResourceCard> initialResourceAllocation(Player player, Intersection settlement, Game game1) {
	...
	if (projectX.Board.coordInRange(nearbyHex)) {
...
public static projectX.Board getMeABoard() {
	
	int Edgesize = 3;
	projectX.Board board1 = getHexBoard(Edgesize);
...
public static projectX.Board getHexBoard(int Edgesize) {
	
	projectX.Board board1 = new projectX.Board();
...
public static projectX.Board setRandHexNumbersAndRob(projectX.Board board1, int givenDesert) {
	...
	while (robL != null) {
		...
		if (robLoc.getNumber() == -1 && robLoc.getisRobberHere().equals(ROBBER)) {
				...
				//Catan.out1.print("\n"+"The robber was left out "+robLoc.getisRobberHere()+robLoc.getNumber());
			robLoc.setNumber(7);
...
public static void setHexNumberTest(Location thisLoc, projectX.Board board1, Iterator normIt, int xc, int yc) {
...
public static void setUpLocations(projectX.Board board1) {
...
public static void setUpInter(int x, int y,  Location[][] boardLocations, projectX.Board board1) {
...
public static void makeIntersectionOrdering(projectX.Board board1) {
...
public static void setRoadArrayAndMap(projectX.Board board1) {
...
public static HashMap makeRoadHashMap(projectX.Board board1) {
...
public static void addPorts(projectX.Board board1) {
...
public static void setIllegals(projectX.Board board1) {
...
public static Intersection isOwned(projectX.Board board1, int j, int g) {
